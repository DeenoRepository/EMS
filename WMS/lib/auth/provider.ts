  ...
  async authenticateFromLogin(login: string, password?: string): Promise<ExternalIdentity | null> {
    ...
    if (this.useDirectBind()) {
      ...
      const bindCandidates = Array.from(
        new Set([
          normalized,
          `cn=${userPart},${this.config.baseDn}`,
          `uid=${userPart},${this.config.userBaseDn}`,
          `cn=${userPart},${this.config.userBaseDn}`
        ].filter(Boolean))
      );

      for (const bindIdentity of bindCandidates) {
        const client = new Client({ url: this.config.url, timeout: 8000, connectTimeout: 8000 });
        try {
