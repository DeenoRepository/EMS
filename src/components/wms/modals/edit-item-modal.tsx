    ...
    <Modal isOpen={isOpen} onClose={onClose} size="lg">
      ...
      <form onSubmit={handleSubmit} className="p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            <label className="block text-xs font-medium text-slate-700 mb-1">Склад размещения (заблокирован) *</label>
            <select
              value={formData.warehouse}
              disabled={true}
              onChange={(e) => setFormData({ ...formData, warehouse: e.target.value })}
              className="w-full rounded-lg border border-slate-200 px-3 py-2 text-xs bg-slate-100 text-slate-500 font-semibold cursor-not-allowed"
            >
              {warehouses.map((w) => (
