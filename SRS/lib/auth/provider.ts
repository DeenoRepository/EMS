...
class MockProvider implements AuthProvider {
  ...
  async authenticate(): Promise<ExternalIdentity | null> {
    ...
    const email = cookieStore.get(AUTH_COOKIE)?.value || cookieStore.get(DEMO_COOKIE)?.value || headerStore.get("x-user-email") || "admin@ems.local";
...
class LdapProvider implements AuthProvider {
  ...
  private identityFromLogin(login: string): ExternalIdentity {
    ...
    const email = normalized.includes("@") ? normalized : `${normalized}@ems.local`;
    return {
      externalId: normalized,
      email,
      displayName: email.split("@")[0] || normalized,
      groups: []
    };
  }
  ...
  private async findUserByLogin(client: Client, login: string) {
    ...
    const email = (this.getAttr(user, "mail") || this.getAttr(user, "uid") || login).trim().toLowerCase();
