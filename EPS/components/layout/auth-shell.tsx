...
const viewerAllowedRoutes = ["/dashboard", "/equipment", "/documents", "/change-history"];
...
export function AuthShell({ children }: { children: ReactNode }) {
  ...
  const isViewerOnly = hasAnyRole(user, ["VIEWER"]) && !hasAnyRole(user, ["EDITOR", "APPROVER", "ADMIN"]);
  if (isViewerOnly && !isAllowedForViewer(pathname)) {
    return <ErrorState text="Для роли VIEWER доступ разрешен только к разделам: Панель управления, Оборудование, Документы, История изменений." />;
  }
